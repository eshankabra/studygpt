...
import {
    CheckCircle, FileText, Layers, MessageSquare,
    ChevronLeft, ChevronRight
} from 'lucide-react';

const tools = [

    { icon: CheckCircle, label: 'AI Task Manager', id: 'tasks', path: '/task-manager' },
...
const ToolTabs = () => {
    ...
    return (
        <div className="max-w-7xl mx-auto px-6 mb-16 relative group">
            ...
            <div
                ref={scrollRef}
                className="flex items-center md:justify-center gap-4 overflow-x-auto pb-4 no-scrollbar border-b border-slate-800/50 scroll-smooth"
            >
                {tools.map((tool) => (
                    <Link
                        key={tool.id}
                ...
                ${location.pathname === tool.path && (tool.id === 'notes' || tool.id === 'flashcards' || tool.id === 'tasks')
                                    ? 'bg-blue-500/10 border-b-2 border-blue-500 text-white'
                                    : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}
              `}
                        >
                            <tool.icon size={18} className={(location.pathname === tool.path && (tool.id === 'notes' || tool.id === 'flashcards'|| tool.id === 'tasks')) ? 'text-blue-400' : ''} />
